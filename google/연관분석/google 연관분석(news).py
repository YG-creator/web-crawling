...
def crawler(maxpage,query,s_date,e_date):
    ...
    while page <=maxpage_t:
        url = 'https://www.google.com/search?q=' + query  + '&tbm=nws' + "&tbs=cdr%3A1%2Ccd_min%3A"  + s_date[0] + "%2F" + s_date[1] + "%2F" + s_date[2] + "%2Ccd_max%3A" + e_date[0] + "%2F" + e_date[1] + "%2F" + e_date[2] + "&start=" + str(page)
        driver = webdriver.Chrome()
        driver.get(url)
        html = driver.page_source
        soup = BeautifulSoup(html,"html.parser")

        list = soup.select('.KWQBje')
        for i in list:
            title_list.append(i.select_one('.JheGif.nDgy9d').text)
        driver.close()
        page += 10
...
plt.savefig('C:/Users/YG/Desktop/test/MG02.png',bbox_inches='tight')
plt.show()
